// 1. Smooth scroll for nav links (excluding contact link)
document.querySelectorAll('nav a').forEach(link => {
  if (link.id !== 'navContact') {
...
// 2. Make "Contact" in nav open modal
document.getElementById('navContact').addEventListener('click', (e) => {
  e.preventDefault();
  openModal();
});

// 3. "Get Started" buttons open modal
document.getElementById('getStartedBtn').addEventListener('click', () => {
  openModal();
});
document.getElementById('footerGetStarted').addEventListener('click', () => {
  openModal();
});
...
// 5. Review slider: auto slide every 4 seconds
...
// 6. Toggle review details (accordion)
